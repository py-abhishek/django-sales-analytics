...
from reports.services.product_analysis import get_top_categories, get_top_selling_products
from reports.services.sales_analysis import get_sales_trend


def get_insights(business_id, year=None, month=None):

    sales, expenses, sale_items = get_filtered_queries(business_id, year, month)
...
def get_filtered_queries(business_id, year=None, month=None):
    ...
    if year:
        ...
        sale_items = sale_items.filter(sale__sale_date__year=year)

    if month:
        sales = sales.filter(sale_date__month=month)
        expenses = expenses.filter(expense_date__month=month)
        sale_items = sale_items.filter(sale__sale_date__month=month)


    return sales, expenses, sale_items
